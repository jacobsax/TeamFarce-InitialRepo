...
/**
 * The InterviewResponseBox is used within the InterviewScreen Used to present the player with
 * options for responding to a Suspect
 */
public class InterviewResponseBox {
     ...
     */
    public InterviewResponseBox(
        String content,
        ArrayList<InterviewResponseButton> buttonList,
        Skin uiSkin,
        boolean isMultiRow
    ) {
        textContent = content;
    ...
    /**
     * This method returns a content table for the Interview screen. It does so based on what needs
     * to be showed on it
     *
    ...
    public Table getContent() {
        ...
        if (buttonCount > 0) {
            ...
            for (int i = 0; i < buttonCount; i++) {
                ...
                buttonElement.addListener(
                    new ClickListener() {
                        @Override
                        public void clicked(InputEvent event, float x, float y) {
                            // Trigger click event handler for current button (see button
                            // definition)
                ...
                if (isMultiRow) {
                    table.add(buttonElement).width((TABLE_WIDTH - (2 * PADDING)) - (PADDING)).pad(
                        PADDING,
                        PADDING / 2,
                        0,
                        PADDING / 2
                    ...
                    table.add(buttonElement).width(buttonWidth).pad(
                        PADDING,
                        PADDING / 2,
                        0,
                        PADDING / 2
