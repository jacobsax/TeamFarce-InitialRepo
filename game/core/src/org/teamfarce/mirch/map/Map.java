...
public class Map {
    ...
    public List<Room> initialiseRooms() {
        ...
        mainRoom
            ...
            .addTransition(
                new Room.Transition().setFrom(12, 1).setTo(rch037, 3, 5, Direction.SOUTH)
            ); // To RCH/037

        rch037
            .addTransition(
                new Room.Transition().setFrom(2, 5).setTo(mainRoom, 11, 1, Direction.NORTH)
            ) // To Main Room
            .addTransition(
            ...
            .addTransition(
                new Room.Transition().setFrom(12, 17).setTo(computerRoom, 8, 1, Direction.NORTH)
            ); // To Computer Room

        portersOffice.addTransition(
            new Room.Transition().setFrom(1, 5).setTo(mainRoom, 17, 17, Direction.WEST)
        ); // To Main Room

        kitchen.addTransition(
            new Room.Transition().setFrom(1, 3).setTo(mainRoom, 27, 13, Direction.WEST)
        ); // To Main Room

        islandOfInteraction.addTransition(
            new Room.Transition().setFrom(11, 14).setTo(mainRoom, 26, 29, Direction.WEST)
        ); // To Main Room

        toilet.addTransition(
            new Room.Transition().setFrom(1, 1).setTo(mainRoom, 18, 2, Direction.WEST)
        ); // To Main Room
        ...
        lakeHouse.addTransition(
            new Room.Transition().setFrom(11, 1).setTo(computerRoom, 13, 11, Direction.WEST)
        ); // To Computer Room

        outside
            .addTransition(
                new Room.Transition().setFrom(19, 4).setTo(mainRoom, 3, 5, Direction.NORTH)
            ) // To Main Room
            .addTransition(
        ...
        pod
            .addTransition(
                new Room.Transition().setFrom(18, 9).setTo(outside, 9, 11, Direction.EAST)
            ) // To Outside
            .addTransition(
    ...
    /**
     * This method takes a list of NPCs and then randomly distibutes them around the rooms of the
     * map
     *
    ...
    public void placeNPCsInRooms(List<Suspect> NPCs) {
        ...
        for (Suspect loopNpc: NPCs) {
            /*
             * Refill the rooms left list if there are more NPCs than Rooms. This will put AT LEAST
             * one NPC per room if so.
             */
            ...
            System.out.println(
                loopNpc.getName() + " has been placed in room " + selectedRoom + " at " + position
            );
        }
    }
